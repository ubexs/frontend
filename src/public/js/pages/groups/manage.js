...
`);
    ...
    // load group funds data
    // (this used to happen on the backend...)
    request('/group/'+groupid+'/funds', 'GET').then(funds => {
       $('#group-funds-primary').text(number_format(funds.Primary));
       $('#group-funds-secondary').text(number_format(funds.Secondary));
    });
}