...
function loadTrades(type, offset) {
    ...
    request("/staff/economy/trades/"+type+"/?offset="+offset+'&userId='+currentUserId, "GET")
        .then(function(data) {
            if (data.length <= 0) {
...
$(document).on('click', '.reviewTradeData', function() {
    ...
    request("/staff/economy/trades/"+id+"/items?userId="+currentUserId, "GET")
        .then(function(d) {
            var itemsToGiveHtml = ``;
                ...
                if (type === "outbound") {
                    ...
                    if (d.dismiss && d.dismiss === "backdrop") {
                        ...
                        questionYesNo("Are you sure you want to cancel this trade?", function() {
                            ...
                            $('#current-trade-card-body').empty();
                            hideTradeBtn();
                            request("/staff/economy/trades/"+id+'?userId='+currentUserId, "DELETE")
                                .then(function(d) {
                ...
                if (type === "inbound") {
                    if (d.dismiss && d.dismiss === "backdrop") {
                        ...
                        questionYesNo("Are you sure you want to accept this trade?", function() {
                            ...
                            $('#current-trade-card-body').empty();
                            hideTradeBtn();
                            request("/staff/economy/trades/"+id+'?userId='+currentUserId, "POST")
                                .then(function(d) {
